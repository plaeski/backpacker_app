...
class Calculator
  ...
  def calculate_percents (money_arr)
    ...
    should_be_hundred_ish = (accommo_p+food_p+local_transit_p+alcohol_p).round(0)
  ...
  def show_country_suggestion (days, budget)
    ...
    min_cost = 0 if min_cost < 0  # [ min_cost, max_cost ]
    ...
    all_currency = all_currency_code.map { |z| convert_currency(z) } # this takes time due to google_currency
    
    i = 0
    result = []
    while i < num.length
      cad = (local_expense_arr[i]/all_currency[i]).round(2)
      if cad >= min_cost && cad <= max_cost
        result.push([cad, i])
      end
      i += 1
    end
